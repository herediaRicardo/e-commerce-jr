...
const renderPage = (page) => {
    ...
    const content = productsToShow.map(({ img = './assets/img/default.webp', item = 'Producto no disponible', price = 0 }) => {
        const formattedPrice = price.toLocaleString();
        return `
            <div class="product">
                <img src=".${img}" alt="${item}">
...
document.getElementById('btn-carrito').addEventListener('click', (e) => {
    ...
    modalCart.classList.remove('cart-none');
    modalCart.scrollIntoView({ behavior: 'smooth', block: 'start' });
});
